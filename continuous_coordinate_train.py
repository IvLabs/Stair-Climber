...
class env():
    ...
    def State(self):
        ...
        return pose,messg,stero

    def Statedict(self,pose,messg,stero):
        ...
        state[7]=np.array([messg.linear.x,messg.linear.y,messg.linear.z])
    ...
    def reward_function(self,state):
        ...
        if self.wallx[0]<=x<=self.wallx[1] and self.wally[0]<=y<=self.wally[1] and self.wallz[0]<=z<=self.wallz[1]:
            if dist<=0.01:
                return  1 #and dist2<0.01: #and  if sum of image_data10000*25
            else:
                return 0
        else:
            return 0
    ...
    def done(self,state):
        ...
        z=state[2]
        if self.wallx[0]<=x<=self.wallx[1] and self.wally[0]<=y<=self.wally[1] and self.wallz[0]<=z<=self.wallz[1]:
            if self.reward_function(state)==10:
                return True
            else:
                return False
        else: 
    ...
    def _step(self,action):
        ...
        nextpose,nextmessg,nextstero=self.State()
        nextstate=self.Statedict(nextpose,nextmessg,nextstero)
    ...
    def _reset(self):
        ...
        pose,messg,stero=self.State()
        state=self.Statedict(pose,messg,stero)
        goal_pose,goal_messg,goal_stero=self.State()
        goal_state=self.Statedict(pose,messg,stero)
        #goal_state+=np.random.choice(2)
        rospy.wait_for_service('/gazebo/pause_physics')
        try:
